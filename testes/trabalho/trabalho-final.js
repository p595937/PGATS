...
describe('Testes da disciplina - fundamentos JS', () => {
  ...
  it('QUANDO informar um nome para o Pet, DEVE ser impresso na tag com letras secretas', () => {
    assert.strictEqual(geradorDeTagsDeIdentificacao('Pantera'), 'PANTERA');
  });

  it('QUANDO a idade = 1 + porta M, DEVE ser permitida a adoção', () => {
    assert.strictEqual(verificarSePodeSerAdotado(1, 'M'), true);
  });

  it('QUANDO o peso = 14.5, DEVE ser devolvido 4350 gramas para o consumo diário', () => {
    assert.strictEqual(calcularConsumoDeRacao('Pitoco', 1, 14.5), 4350);
  });

  it('QUANDO o porte = pequeno, DEVE ser retornado a atividade adequada', () => {
